...
export default function LeafParticles({
  ...
  const drawLeaves = useCallback(() => {
    ...
    while (leaves.current.length < quantity) {
      ...
      // Find the max existing ID and add from there
      const maxId = leaves.current.reduce(
        (max, leaf) => Math.max(max, leaf.id),
        -1
      );
      leaves.current.push(leafParams(maxId + 1));
    }
    // Remove excess leaves if quantity decreased
    // Filter out stopped leaves first if we want to preserve them on slice
    ...
    if (canvasContainerRef.current && canvasRef.current && context.current) {
      ...
      leaves.current.forEach((leaf) => {
        if (!leaf.stopped) {
          ...
          // Reset y to initialY (above or at original start) and randomize dy
          // This prevents weird behavior if the canvas height shrinks significantly
          leaf.y = leaf.initialY;
          leaf.dy = Math.random() * fallSpeed * 0.5;
        }
      });

      // After resetting falling leaves, adjust quantity and redraw
      drawLeaves();
    }
  }, [dpr, fallSpeed, drawLeaves]); // Added drawLeaves dependency, removed quantity, leafParams as drawLeaves handles quantity adjustment

  const onMouseMove = useCallback(() => {
    if (canvasRef.current) {
      const rect = canvasRef.current.getBoundingClientRect();
      // Mouse position relative to the canvas element's top-left corner
  ...
  const animate = useCallback(() => {
    ...
    const updateLeaves = () => {
      // It's generally safer to iterate over a copy if you modify the array within the loop,
      // but since we are only modifying leaf properties and not adding/removing within this loop,
      // iterating directly over leaves.current is fine and avoids array copy overhead.
      const currentLeaves = leaves.current;

      for (let i = currentLeaves.length - 1; i >= 0; i--) {
        // Iterate backwards (good practice, although not strictly needed for this loop's logic)
        ...
        if (!leaf.stopped) {
          ...
          if (leaf.y + leaf.height / 2 >= canvasSize.current.h) {
            ...
            // Iterate over a subset (e.g., nearby leaves) for performance in very large quantities
            // For simplicity here, we iterate over all stopped leaves
    ...
    // Dependencies for animate: mouse, canvasSize are refs, their .current is accessed.
    // drawLeaf, clearContext are useCallback functions used within render.
    // fallSpeed, windStrength are props used in updateLeaves.
  }, [fallSpeed, windStrength, mouse, canvasSize, drawLeaf, clearContext]);
  ...
  useEffect(() => {
    ...
    if (currentLeafCount < quantity) {
      // Find the max existing ID to ensure new IDs are unique
      const maxId = leaves.current.reduce(
        (max, leaf) => Math.max(max, leaf.id),
        -1
      );
      for (let i = currentLeafCount; i < quantity; i++) {
        leaves.current.push(leafParams(maxId + 1 + (i - currentLeafCount)));
      }
  ...
  }, [quantity, leafParams, drawLeaves]);
