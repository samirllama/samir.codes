...
const projects: ProjectEntry[] = experience.map((e) => ({
  ...
  projectScreenshotUrl: e.projectScreenshotUrl || "/placeholder.svg",
}));
...
const TimelinePage = () => {
  ...
  return (
    <div
      ref={containerRef}
      className="bg-hdr-gradient text-white font-sans p-4 sm:p-6 lg:p-8"
    >
  ...
  return (
    <div className="timeline-entry relative flex items-start mb-16">
      ...
      <div className="timeline-meta flex-shrink-0 w-28 sm:w-auto sm:absolute sm:left-1/2 sm:transform sm:-translate-x-full sm:pr-8 text-right">
        <div className="timeline-dot absolute left-4 sm:left-auto sm:right-0 top-1 w-3 h-3 rounded-full bg-gray-500 transform sm:translate-x-1/2 glow-animation"></div>
...
const ProjectCard = ({
  ...
  const finalScreenshotUrl =
    projectScreenshotUrl ||
    "https://placehold.co/600x450/333333/FFFFFF?text=Image+Not+Available";
    ...
    <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-lg overflow-hidden transition-all duration-300 ease-in-out hover:scale-[1.02] hover:shadow-lg hover:shadow-cyan-500/20">
      <div className="relative w-full pt-[60%] bg-gray-800">
        <Image
          ...
          onError={(e) => {
            e.currentTarget.src =
              "https://placehold.co/600x450/333333/FFFFFF?text=Image+Load+Error";
          }}
        />
      </div>
      <div className="p-6">
