...
import { withRateLimit, validateFormData, ActionResponse } from './utils';
...
async function signInAction(prevState: ActionResponse, formData: FormData): Promise<ActionResponse> {
...
async function signUpAction(prevState: ActionResponse, formData: FormData): Promise<ActionResponse> {
  ...
  try {
    ...
    const [newUser] = await db.insert(users).values({ id, email, password: hashedPassword }).returning();
    if (!newUser) {
